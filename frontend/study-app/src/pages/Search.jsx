...
const Search = () => {
  ...
  useEffect(() => {
    const fetchBook = async () => {
      try {
        const res = await axios.get(`https://study-app-2-82ch.onrender.com/book?search=${query}`);
        setBook(res.data);
      } catch (error) {
...
const fetchquestions=async()=>{
    try {
      const res=await axios.get(`https://study-app-2-82ch.onrender.com/interview?search=${query}`);
